...
depth <- vroom("../../data/1000-genomes/benchmark-1000genomes-nfhlamajority-local-update-db-exclude-trim/benchmark-1000genomes-nfhlamajority-kourami-3-63-0-skip-trim-1002-samples/combined_results/nf_hlamajority_depth_sorted.tsv")
#depth <- depth %>% distinct(mean_depth_hla_exons_2_3_classI, .keep_all = T) 
all.in <- read.table("../../data/1000-genomes/benchmark-1000genomes-nfhlamajority-local-update-db-exclude-trim/benchmark-1000genomes-nfhlamajority-kourami-3-63-0-skip-trim-1002-samples/combined_results/nf_hlamajority_all_calls_sorted.tsv", sep = "\t", header = T)
results <- readRDS("data/results/hlamajority/1000-genomes-1002samples-full-results-hlamajority-majority-vote.Rds")
...
df <- read.csv("data/results/hlamajority/1000-genomes-1002samples-full-stats-hlamajority-majority-vote.csv")
...
# 4. Display Plot
print(p)
ggsave(plot = p, filename = "data/results/hlamajority/plots/hlamajority-accuracy-per-gene-1000genomes-1012samples.png", width = 10, height = 10)
ggsave(plot = p, filename = "data/results/hlamajority/plots/hlamajority-accuracy-per-gene-1000genomes-1012samples.svg", width = 10, height = 10)
...
# 4. Display Plot
print(p_1row)
ggsave(plot = p_1row, filename = "data/results/hlamajority/plots/hlamajority-accuracy-per-gene-1000genomes-1012samples-1row.png", width = 15, height = 10)
ggsave(plot = p_1row, filename = "data/results/hlamajority/plots/hlamajority-accuracy-per-gene-1000genomes-1012samples-1row.svg", width = 15, height = 10)
...
ggsave(plot = compare_hlamajority_claeys, filename = "data/results/hlamajority/plots/compare-claeys-hlamajority-1000genomes-1012samples.png", width = 15, height = 10)
ggsave(plot = compare_hlamajority_claeys, filename = "data/results/hlamajority/plots/compare-claeys-hlamajority-1000genomes-1012samples.svg", width = 15, height = 10)
...
ggsave(plot = p_call, filename = "data/results/hlamajority/plots/hlamajority-call-rate-per-gene-1000genomes-1002samples.svg", width = 15, height = 8)
ggsave(plot = p_call, filename = "data/results/hlamajority/plots/hlamajority-call-rate-per-gene-1000genomes-1002samples.svg", width = 15, height = 8)
...
fit_mixed <- glmer(is_called ~ mean_depth_hla_exons_2_3_gene +
                     (1 | sample),
                   data = all.in.kourami.depth,
                   family = binomial)
exp(0.06125)
fit_mixed

glmer(is_called ~ mean_depth_hla_exons_2_3_gene + gene + (1 | sample),
      data = all.in.kourami.depth,
      family = binomial)

glmer(is_called ~ mean_depth_hla_exons_2_3_gene +
        (1 + mean_depth_hla_exons_2_3_gene | sample),
      data = all.in.kourami.depth,
      family = binomial)

all.in.kourami.depth %>% group_by(gene) %>% summarise(n_called = sum(is_called))

# Sequence of depths covering your observed range
depth_seq <- seq(
  min(all.in.kourami.depth$mean_depth_hla_exons_2_3_gene, na.rm = TRUE),
  max(all.in.kourami.depth$mean_depth_hla_exons_2_3_gene, na.rm = TRUE),
  length.out = 200
)
all.in.kourami.depth$gene <- factor(all.in.kourami.depth$gene)
# Expand for each gene
newdat <- expand.grid(
  mean_depth_hla_exons_2_3_gene = depth_seq,
  gene = levels(all.in.kourami.depth$gene)
)

fit_gene <- glmer(is_called ~ mean_depth_hla_exons_2_3_gene + gene + (1 | sample),
                  data = all.in.kourami.depth,
      family = binomial)

newdat$predicted_prob <- predict(
  fit_gene,              # your glmer model with gene
  newdata = newdat,
  type = "response",
...
ggplot(newdat,
       aes(x = mean_depth_hla_exons_2_3_gene,
           y = predicted_prob,
           colour = gene)) +
  geom_line(size = 1.2) +
  labs(x = "Mean HLA exon depth",
       y = "Predicted probability of genotype call",
       colour = "Gene") +
  theme_classic()

all.in.kourami.depth$is_called_num <-
  as.numeric(as.character(all.in.kourami.depth$is_called))

ggplot() +
  geom_jitter(data = all.in.kourami.depth,
              aes(x = mean_depth_hla_exons_2_3_gene,
                  y = is_called_num,
                  colour = gene),
              alpha = 0.2,
  ...
  geom_line(data = newdat,
            ...
            size = 1.2) +
  labs(x = "Mean HLA exon depth",
       y = "Probability of genotype call") +
  theme_classic()

depth_seq <- seq(
  min(all.in.kourami.depth$mean_depth_hla_exons_2_3_gene),
  max(all.in.kourami.depth$mean_depth_hla_exons_2_3_gene),
  length.out = 1000
)
...
threshold_df$pred <- predict(
  fit_gene,
  newdata = threshold_df,
  type = "response",
  re.form = NA   # fixed effects only
)

thresholds_95 <- threshold_df %>%
  group_by(gene) %>%
  filter(pred >= 0.95) %>%
  summarise(depth_95 = min(mean_depth_hla_exons_2_3_gene))
...
# add depth to scores
# read in scores
scores <- read.csv("data/results/hlamajority/1000genomes-score-per-sample.csv")
scores
depth_scores <- left_join(scores, depth)

install.packages("ordinal")  # if not already installed
# Make Score an ordered factor
depth_scores$Score_ord <- factor(depth_scores$Score, levels = 0:2, ordered = TRUE)
fit_score_mixed <- clmm(
  Score_ord ~ mean_depth_hla_exons_2_3_gene + gene + tool + (1 | sample),
  data = depth_scores,
  Hess = TRUE
)
summary(fit_score_mixed)
fit_score_mixed
...
ggplot(depth_scores, aes(x = gene, y = mean_depth_hla_exons_2_3_gene, fill = correct_flag)) +
...
p_gene <- ggplot(depth_scores, aes(x = correct_flag, y = mean_depth_hla_exons_2_3_gene, fill = correct_flag)) +
...
ggsave(plot = p_gene_adjusted, filename = "data/results/hlamajority/plots/compare-hlamajority-accuracy-per-gene-correct-incorrect-1000genomes-1012samples.png", width = 10, height = 7)
...
ggsave(plot = p_gene_adjusted_gene_tool, filename = "data/results/hlamajority/plots/compare-hlamajority-accuracy-per-gene-correct-incorrect-1000genomes-1012samples-facet-gene-tool.png", width = 10, height = 7)
ggsave(plot = p_gene_adjusted_gene_tool, filename = "data/results/hlamajority/plots/compare-hlamajority-accuracy-per-gene-correct-incorrect-1000genomes-1012samples-facet-gene-tool.svg", width = 10, height = 7)

# table of medians per group for reference
median_depth_correct_incorrect <- depth_scores %>%
